...
export default async function HeadToHeadPage({ searchParams }: HeadToHeadPageProps) {
  ...
  const defaultPlayerA = params.playerA ?? players[0]?.id;
  const defaultPlayerB = params.playerB ?? players[1]?.id;
    ...
    <div className="grid gap-6">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Pick any two players to compare results, goals, and points from direct meetings.
        </p>
      ...
      {players.length < 2 ? (
        <div className="rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-8 text-sm text-slate-500">
          Add at least two players to unlock head-to-head comparisons.
        </div>
      ) : playerA && playerB && playerA.id === playerB.id ? (
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 space-y-3">
              {relevantMatches.length === 0 ? (
                <p className="text-sm text-slate-500">These players have not met yet.</p>
              ) : (
