      ...
      <section className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
        <div className="rounded-3xl bg-slate-950 p-8 text-white shadow-sm">
          ...
          <h1 className="mt-4 text-3xl font-bold tracking-tight sm:text-4xl">
            Live football standings powered entirely by recorded match results.
          </h1>
          <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-300 sm:text-base">
            Add players, record scores, and let the app calculate positions, points per match,
            goal difference, and the custom rating formula automatically.
          </p>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.6fr_1fr]">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Link href="/table" className="text-sm font-medium text-sky-700 hover:text-sky-600">
              View full table
            </Link>
          </div>
          <LeagueTable rows={table.slice(0, 8)} compact />
        </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500">Latest recorded results.</p>
              </div>
              <Link href="/history" className="text-sm font-medium text-sky-700 hover:text-sky-600">
                See history
              </Link>
            </div>
            <div className="mt-5 space-y-3">
              {matches.length === 0 ? (
                <p className="rounded-2xl border border-dashed border-slate-300 px-4 py-5 text-sm text-slate-500">
                  No matches recorded yet. Start by adding players and a result.
                </p>
              ) : (
